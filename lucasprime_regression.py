...
lp_data = pd.read_csv("FibPrime_2.csv")
...
x = lp_data["x5"][:21].values.reshape(-1, 1)
y1 = lp_data["log(log(lucas_primes))"][:21].values.reshape(-1, 1)
...
plt.xlabel("N")
plt.ylabel("Log(Log(Lucas Primes))")
plt.legend(["Log(Log(Lucas Primes))", "Best Fit Line", "Best Fit Cubic Polynomial"])
plt.show()
